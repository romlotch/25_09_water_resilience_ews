...
def compute_accumulated_percent(ds, cp_name, pval_name,
    ...
    # Only cells with a valid cp index and significant p-value
    sig = (pval < 0.05) & (cp > 0)
    ...
    # Total valid land area = where cp is not NaN
    valid = cp.notnull()
    total_area = float(area_da.where(valid).sum().values)
    if total_area <= 0: # if none return empty 
        return np.array([], dtype=int), np.array([], dtype=float)

    # Convert cp index to year, then to integer bins
...
def extend_series_to(years_base, years, vals):
    """ 
    Extend axis in case one is shorter
    """
...
# --- plotting ---
def plot_accumulated_three(sm_ds_path, et_ds_path, p_ds_path,
                           test_name, outdir): 
    # Map test to var names
